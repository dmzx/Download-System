	...
	*/
	...
	public function check($return_version = false)
	{
		...
		if ($allow_url_fopen)
		{
			...
			$template_data = array(
				'EDS_AUTHOR'				=> $this->version_data['author'],
				'EDS_CURRENT_VERSION'		=> $this->current_version,
				'EDS_UP_TO_DATE'			=> sprintf((!$version_up_to_date) ? $this->user->lang['EDS_NOT_UP_TO_DATE'] : $this->user->lang['EDS_UP_TO_DATE'], $this->version_data['title']),
				'EDS_S_UP_TO_DATE'			=> $version_up_to_date,
				'EDS_U_AUTHOR'				=> 'http://www.phpbb.com/community/memberlist.php?mode=viewprofile&un=' . $this->version_data['author'],
				'EDS_TITLE'					=> (string) $this->version_data['title'],
				'EDS_LATEST_VERSION'		=> $this->current_version,
			);
			...
			if (!$version_up_to_date)
			{
				...
				$template_data = array_merge($template_data, array(
					'EDS_ANNOUNCEMENT'		=> (string) $updates['announcement'],
					'EDS_DOWNLOAD'			=> (string) $updates['download'],
					'EDS_LATEST_VERSION'	=> $updates['current'],
				));
			}
